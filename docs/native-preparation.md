...
实时依据是**设备上的** `/var/lib/dpkg/status`，与由哪台电脑或哪个包管理器安装无关。普通扫描通过可用的 AFC2 或已授权 SSH 会话读取它；扫描不会自动尝试 root 密码。应用重启后 SSH 会话消失且 AFC2 不可用时，实时结果是未知，不是未安装。

接入条件页新增“验证 AppSync”，以用户输入的设备 root SSH 密码进行一次只读验证。独立命令 `check_appsync` 不调用准备方案、包安装、服务重载或重启。验证后的 SSH 会话供本次连接期间只读刷新使用，密码不落盘。断开、重新启动应用或读取失败后仍需区分实时结果与历史记录。

`environment.sqlite` 位于 Studio 的应用数据目录，仅记录设备标识摘要、上次安装状态和观察时间；它不存储密码，也不作为安装兼容性的实时依据。未知读取保留历史，界面显示“上次已安装 · 待确认”；只有新读取的设备记录才能显示当前已安装或未安装。未知状态提供只读验证入口，确认未安装后才引导准备。来自旧版本的安装没有历史观察，需要执行一次验证建立记录。

2026-09-07 在当前 iPod4,1 / iOS 6.1.6 上运行只读硬件验证：独立 SSH 检测确认 AppSync 已安装；丢弃会话并重新创建 probe 后，实时结果未知但“上次已安装”的记录保留。整个验证没有执行安装、重载或重启。

```sh
cargo test --manifest-path src-tauri/Cargo.toml --lib read_only_appsync_verification_preserves_history_after_session_restart -- --ignored --nocapture
```

该测试要求只连接目标设备；默认测试密码为 alpine，可通过 `POCKET_TEST_SSH_PASSWORD` 提供改过的密码。常规测试不连接设备，覆盖历史持久化、不同设备隔离、读取失败保留历史、实时缺失覆盖历史、历史不得算作就绪，以及界面只读验证命令。
