...
原生方案先下载和构建，再请求用户进入 DFU。概览列出步骤、系统影响、风险和取消边界。授权版本为 `2026-09-06`，包含 root ramdisk 会话、分区挂载、Aquila 6 / Cydia / OpenSSH 安装；原来的“仅用于演示”条款已移除。浏览器模式另行显示模拟提示。
...
2. 重新验证设备、配对、电量与固件，等待用户手动进入 DFU。
